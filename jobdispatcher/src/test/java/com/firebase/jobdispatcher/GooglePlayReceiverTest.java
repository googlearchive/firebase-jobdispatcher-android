...
public class GooglePlayReceiverTest {
  ...
  /**
   * The default ShadowMessenger implementation causes NPEs when using the {@link
   * Messenger#Messenger(Handler)} constructor. We create our own empty Shadow so we can just use
  ...
  public void onJobFinished_successRecurringContentJob_reschedule() {
    JobInvocation jobInvocation =
        receiver.prepareJob(callbackMock, getBundleForContentJobExecutionRecurring());
  ...
  private static void assertResultWasStartNotSticky(int result) {
    assertEquals(

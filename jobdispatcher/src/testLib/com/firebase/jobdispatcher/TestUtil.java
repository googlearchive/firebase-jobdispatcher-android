...
public class TestUtil {
  ...
  private static final JobTrigger[] TRIGGER_COMBINATIONS = {
    ...
    Trigger.contentUriTrigger(
        Arrays.asList(
            new ObservedUri(ContactsContract.AUTHORITY_URI, 0),
  ...
  public static void assertHasSinglePrivateUnusableConstructor(Class<?> cls) {
    try {
  ...
  static void assertJobsEqual(JobParameters input, JobParameters output) {
    ...
    assertEquals(
        "shouldReplaceCurrent()", input.shouldReplaceCurrent(), output.shouldReplaceCurrent());
  ...
  static void assertRetryStrategiesEqual(RetryStrategy in, RetryStrategy out) {
    ...
    assertEquals(prefix + "getPolicy()", in.getPolicy(), out.getPolicy());
    assertEquals(prefix + "getInitialBackoff()", in.getInitialBackoff(), out.getInitialBackoff());
    assertEquals(prefix + "getMaximumBackoff()", in.getMaximumBackoff(), out.getMaximumBackoff());
  }
  ...
  /** A factory for building JobInvocations using the JobParameterBuilder interface. */
  private static class JobInvocationBuilder implements JobParameterBuilder<JobInvocation> {

    @Override
    public JobInvocation build(
        String tag,
        boolean replaceCurrent,
        List<Integer> constraintList,
        boolean recurring,
        int lifetime,
        JobTrigger trigger,
        Class<TestJobService> service,
        Bundle extras,
        RetryStrategy rs) {
  ...
  /** A factory for building Jobs using the JobParameterBuilder interface. */
